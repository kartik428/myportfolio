    ...
    >
      ...
      <div className="flex flex-wrap gap-1.5 mt-3">
        {hireCard.skills.map((s) => (
          <span
            key={s}
            className="text-[10px] bg-green-900/40 text-green-300 border border-green-700/40 px-2 py-0.5 rounded-full"
          >
            {s}
          </span>
        ))}
...
export default function Timeline() {
  ...
  return (
    <section id="experience" className="bg-[#000000] py-16 px-4">
      ...
      {/* ── MOBILE layout (< md) ── line left, all cards right */}
      <div className="block md:hidden relative max-w-lg mx-auto">
        ...
        {/* Top purple badge */}
        <div className="flex justify-start pl-[9px] mb-4">
          <motion.div
        ...
        {timelineData.map((item, i) => (
          <div key={i} className="flex items-start relative mb-8">
            {/* Dot */}
            <div className="absolute left-[9px] top-4 w-3 h-3 rounded-full bg-blue-500 z-10 flex-shrink-0 -translate-x-1/2" />
            ...
            <div className="ml-10 flex-1">
              <motion.div
                ...
                whileHover={{ scale: 1.02 }}
                className="bg-[#1e2a3a] border border-white/10 rounded-xl p-4 cursor-pointer hover:border-blue-500 transition-colors duration-200"
              >
                <p className="text-sm font-bold text-white">{item.title}</p>
                <p className="text-xs text-cyan-400 mt-0.5 font-medium">{item.subtitle}</p>
                <div className="flex flex-col gap-0.5 mt-2">
        ...
        {/* Hire Me Flag */}
        <div className="flex justify-start pl-10 mb-4">
          <motion.span
            initial={{ opacity: 0 }}
        ...
        <div className="flex items-start relative mb-8">
          {/* Glowing green dot */}
          <div className="absolute left-[9px] top-4 w-3 h-3 rounded-full bg-green-400 z-10 flex-shrink-0 -translate-x-1/2 shadow-[0_0_8px_2px_rgba(74,222,128,0.6)]" />
          ...
          <div className="ml-10 flex-1">
            <motion.div
              ...
              transition={{ duration: 0.5, ease: "easeOut", delay: 0.1 }}
              whileHover={{ scale: 1.02 }}
              className="bg-[#0f2a1e] border border-green-500/30 rounded-xl p-4 cursor-pointer hover:border-green-400 transition-all duration-200"
            >
              <div className="flex items-center gap-2 mb-1">
                <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                <p className="text-sm font-bold text-white">{hireCard.title}</p>
      ...
      {/* ── DESKTOP layout (md+) ── center line, alternate left/right ── */}
            ...
            <div key={i}>
              <div className="flex items-center relative mb-10">
                ...
                <motion.div
                  ...
                  className="absolute left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-blue-500 z-10"
                />
